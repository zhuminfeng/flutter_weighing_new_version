...
import 'subsystem_config_screen.dart' show detectSlaveRole, slaveRoleIcon, slaveRoleColor, slaveRoleLabel;
...
class _SubsystemDetailConfigScreenState
  ...
  Future<void> _toggleEnabled() async {
    ...
    final ok = await _platform.setSubsystemEnabled(
        _mapping.subsystemId, newEnabled);
    setState(() => _saving = false);
  ...
  Future<void> _assignWeighingDevice() async {
    ...
    final weighingSlaves = widget.slaves
        .where((s) =>
            detectSlaveRole(s.vendorId, s.productCode).name == 'weighing')
        .toList();

    if (weighingSlaves.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(l.noDevicesFound)),
      );
      return;
    }
            ...
            .map(
              (s) => SimpleDialogOption(
                ...
                child: ListTile(
                  ...
                  title: Text(s.userAlias.isNotEmpty
                      ? s.userAlias
                      : s.description),
    ...
    final ok = await _platform.updateSubsystemMapping(
        _mapping.subsystemId, picked.position);
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(ok
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_mapping.description.isNotEmpty
        ...
        children: [
          ...
          Card(
            child: ListTile(
              ...
              subtitle: Text(widget.inputMode == 'ethercat'
                  ? l.ethercatMode
                  : l.shmemMode),
