...
class _SubsystemConfigScreenState extends State<SubsystemConfigScreen> {
  ...
  Future<void> _removeSubsystem(SubsystemMappingInfo mapping) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(ok ? l.removeSuccess : l.removeFailed)),
      );
      if (ok) await _loadInitialData();
    }
  }
  ...
  Future<void> _toggleEnabled(SubsystemMappingInfo mapping) async {
    ...
    final ok = await _platform.setSubsystemEnabled(
        mapping.subsystemId, newEnabled);
    if (mounted) {
      if (ok) {
        setState(() {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l.saveFailedMsg)),
        );
      }
    }
  }
  ...
  Future<void> _editAlias(EthercatSlaveInfo slave) async {
    ...
    if (confirmed == true && mounted) {
      final ok = await _platform.updateSlaveAlias(
          slave.position, controller.text.trim());
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(ok
...
class _SlaveInfoTile extends StatelessWidget {
  ...
  const _SlaveInfoTile(
      {required this.slave, required this.onEditAlias});
    ...
    return Card(
      ...
      child: ListTile(
        ...
        trailing: Row(
          ...
          children: [
            Chip(
              label: Text(slaveRoleLabel(role, l),
              ...
              Column(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: Theme.of(context).textTheme.labelSmall?.copyWith(
                          color:
                              mapping.enabled ? Colors.green : cs.outline,
                        ),
                  ),
                ],
              ),
