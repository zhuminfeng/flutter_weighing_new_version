...
class ELinuxWeighingSystem extends WeighingPlatform {
  ...
  Future<DigitalInputMapConfig> getDigitalInputMap() async {
    try {
      final m = await _channel
          .invokeMapMethod<String, dynamic>('getDigitalInputMap');
  ...
  Future<bool> saveMaterialRecipe(
    ...
    try {
      final result =
          await _channel.invokeMethod<bool>('saveMaterialRecipe', {
        'subsystemId': subsystemId,
        'name': name,
        'appType': appType,
      });
  ...
  Future<bool> loadMaterialRecipe(
    ...
    try {
      final result =
          await _channel.invokeMethod<bool>('loadMaterialRecipe', {
        'subsystemId': subsystemId,
        'recipeId': recipeId,
        'appType': appType,
      });
  ...
  Future<List<Map<String, dynamic>>> getAllMaterialRecipes(int appType) async {
    try {
      final result =
          await _channel.invokeMethod('getAllMaterialRecipes', {
        'appType': appType,
      });
      if (result is List) {
        return result.cast<Map<dynamic, dynamic>>().map((e) {
  ...
  Future<bool> deleteMaterialRecipe(int recipeId, int appType) async {
    try {
      final result =
          await _channel.invokeMethod<bool>('deleteMaterialRecipe', {
        'recipeId': recipeId,
        'appType': appType,
      });
      return result ?? false;
